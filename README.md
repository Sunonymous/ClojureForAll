...
* [`src/clojure4all/`](src/clojure4all/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/clojure4all/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `clojure4all` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/clojure4all/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns clojure4all.example
  (:require [clojure4all.config :as config])
